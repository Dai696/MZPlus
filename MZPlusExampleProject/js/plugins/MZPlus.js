...
Imported.MZPlus = "0.1.0";

/*:
 * @plugindesc <MZPlus> Based off Quasi's amazing QPlus Core.
 * @author Anisoft || Version 0.1.0
 * @target MZ
 *
 * @help
 *
 *
 * The plugin command arg triggerType is reserved. Setting this command as a boolean and to true will have fire off during page start.
 *
...
($ => {
    ...
    $.stringToType = function (string) {
        ...
        const variable = string.match(rx.isVariable)
        if (variable) {
            return {variable: Number(variable[1])}
        }

        const switchMatch = string.match(rx.isSwitch)
        if (switchMatch) {
            return {switch: Number(switchMatch[1])}
        }
...
($ => {
    ...
    $.start = function () {
        if (MZPlus.Core["quickTest"] && Utils.isOptionValid("test")) {
...
($ => {
    $.callCommand = function (self, pluginName, commandName, args, pageRun) {
        ...
        if (typeof func === "function") {
            if (args.triggerType !== 'Page' || pageRun)
                func.call(self, args);
        }
    };
